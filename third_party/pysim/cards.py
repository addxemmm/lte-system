...
class testsim(UsimCard, IsimCard):
	"""
	sysmocom sysmoISIM-SJA2
	"""
	...
	def __init__(self, ssc):
		super(testsim, self).__init__(ssc)
		self._scc.cla_byte = "00"
		self._scc.sel_ctrl = "0004" #request an FCP;
	...
	def autodetect(kls, scc):
		...
		try:
			...
			# 360 card
			atr = "3B 9F 95 80 1F C3 80 31 E0 73 FE 21 13 57 61 11 95 71 25 56 01 09"
			if scc.get_atr() == toBytes(atr):
				return kls(scc)
		except:
			return None
	...
	def program(self, p):
		...
		# Populate AIDs
		print("Start read aids.")
		self.read_aids()

		# update EF-SIM_AUTH_KEY (and EF-USIM_AUTH_KEY_2G, which is
		# hard linked to EF-USIM_AUTH_KEY)
		# print("Start update EF-SIM_AUTH_KEY.")
		# self._scc.select_path(['3f00'])
		# self._scc.select_path(['a515'])
		# if p.get('ki'):
		# 	self._scc.update_binary('6f20', p['ki'], 1)
		...
		# update EF-USIM_AUTH_KEY in ADF.ISIM
		# print("Start update EF-USIM_AUTH_KEY.")
		# data, sw = self.select_adf_by_aid(adf="isim")
		# if sw == '9000':
		# 	if p.get('ki'):
		# 		self._scc.update_binary('af20', p['ki'], 1)
...
_cards_classes = [ FakeMagicSim, SuperSim, MagicSim, GrcardSim,
		   ...
		   FairwavesSIM, OpenCellsSim, WavemobileSim, SysmoISIMSJA2, testsim ]
