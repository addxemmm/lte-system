...
感谢关注。这是一个实验室工具（SDR LTE 小区 + SIM 工具），评审重点：无 shell 注入、API 兼容（`message_id` 语义）、代码与文档同步更新、测试通过。
Thanks for stopping by. This is a lab tool (SDR LTE cell + SIM tooling), so reviews focus on: no shell injection, API compatibility (`message_id` semantics), docs updated alongside code, and tests passing.
...
用 issue 模板（`.github/ISSUE_TEMPLATE/`）：附版本（`VERSION` + `git log --oneline -3`）、`GET /status` + `GET /healthz` 输出、相关容器日志、SDR 型号、用过的频段/参数。真实密钥一律脱敏。
Use the issue templates (`.github/ISSUE_TEMPLATE/`): include version (`VERSION` + `git log --oneline -3`), `GET /status` + `GET /healthz` output, relevant container logs, SDR model, and band/params used. Redact any real keys.
