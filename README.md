...
- 标准 REST：`/api/v1`（正确状态码 + `{"code","message","data","request_id"}` 包络 + OpenAPI，见 `docs/API.md`）
  Standard REST: `/api/v1` (correct status codes + `{"code","message","data","request_id"}` envelope + OpenAPI, see `docs/API.md`)
- 9 个稳定的根路径工具接口（已冻结，见 `docs/API_LEGACY.md`）
  9 stable root-path utility endpoints (frozen, see `docs/API_LEGACY.md`)
- 新增 `GET /healthz`、`GET /status`、`GET /profile`；`/start` 空 body 复用上次配置（持久化见 `docs/RULES.md`）
  New `GET /healthz`, `GET /status` and `GET /profile`; `/start` with an empty body reuses the last saved configuration (persistence, see `docs/RULES.md`)
- 灵活写卡（有写卡器时）：`imsi` 必填，`ki/op/opc/auth/amf/acc/adm/spn/sqn/qci/card/mcc/mnc/iccid` 全可选（见 `docs/SIM.md`）；无写卡器时该接口不可用，不影响入网
  Flexible SIM programming (when a card reader is present): `imsi` is required, `ki/op/opc/auth/amf/acc/adm/spn/sqn/qci/card/mcc/mnc/iccid` are all optional (see `docs/SIM.md`); without a card reader this endpoint is unavailable, network attach is unaffected
- SDR：USRP B210（正版 + BlackSDR 兼容板 FPGA 可切换）与 bladeRF（见 `docs/SDR.md`）
  SDR: USRP B210 (genuine + BlackSDR compat-board FPGA switchable) and bladeRF (see `docs/SDR.md`)
...
See `docs/QUICKSTART.md` for the full network attach flow.
...
- `docs/QUICKSTART.md` — 无写卡器 + 已写卡，直接入网（先看这个） / No card reader + programmed SIM (SIM programming done), direct network attach (start here)
- `docs/RULES.md` — 使用规则：无状态定义、配置持久化、操作流、多终端、升级回滚、射频纪律 / Usage rules: stateless definition, config persistence, operation flow, multi-UE/terminal device, upgrade and rollback, RF discipline
- `docs/API.md` — 接口与 `message_id` 全表 + curl / Endpoints and full `message_id` table + curl
- `docs/DEPLOY.md` — 服务器部署/升级/备份/排障 / Server deployment, upgrade, backup and troubleshooting
- `deploy/docker/README.md` — Docker 专讲：镜像三段构建、compose 逐项解释、版本迭代、构建排障 / Docker deep dive: three-stage image build, compose item-by-item, version iteration, build troubleshooting
- `docs/SIM.md` — 灵活写卡与卡型 / Flexible SIM programming and card types
- `docs/SDR.md` — B210（兼容板 FPGA 切换）与 bladeRF / B210 (compat-board FPGA switching) and bladeRF
- `docs/MIGRATION.md` — 旧 Python → Go 对照 / Old Python vs Go mapping
- `AGENTS.md` — 本地开发规范（subagent / handoff / folk） / Local dev guide (subagent / handoff / folk)

## 参与贡献与许可 Contributing and License

- 想一起改？先看 `CONTRIBUTING.md`（测试要求、API 兼容铁律、PR 模板）
  Want to contribute? Read `CONTRIBUTING.md` first (test requirements, API compatibility rules, PR template)
- 安全问题走 `SECURITY.md` 私密通道；**API 无鉴权，仅限可信局域网**
  For security issues use the private channel in `SECURITY.md`; **the API has no auth, trusted LAN only**
- 本仓库代码 MIT（`LICENSE`）；镜像内含 AGPL-3.0 的 srsRAN 等第三方组件，见 `NOTICE.md` / This repo's code is MIT (`LICENSE`); the image contains third-party components such as srsRAN under AGPL-3.0, see `NOTICE.md`
- 版本历史见 `CHANGELOG.md` / See `CHANGELOG.md` for version history
