...
后续项：小米掉线是否仍可复现尚待单独确认；只读无线样本有欠载/迟到及已完成重建，不将其直接归因于某台 UE。源码复核发现有效 ServiceRequest 的 association 集合登记遗漏，需独立清理回归与发布；当前确认后没有改动运行代码或容器。现场错误 APN 反向测试及长时间稳定性压测仍未执行。下一步先依据用户复现时间关联日志，代码修改再运行 `go test ./...`、`go vet ./...` 与对应 C++ 回归，详见报告。
Follow-ups: clarify whether Xiaomi drops remain reproducible; radio underflow/late events and completed reestablishments have not been attributed to a specific handset outage. A source review found missing association-set registration on valid ServiceRequest, requiring a separate cleanup regression and release. No runtime code or container changed after confirmation. A hardware wrong-APN negative test and long-duration stability/load tests remain outstanding. First correlate logs with user-reported reproduction times; any code change requires Go tests/vet and relevant C++ regressions as detailed in the report.
