...
旧资产对应位置：`legacy-image/`（架构图）、`legacy-tools/`（ACR1281U 手册/驱动）、`legacy-python-workspace/conf/user_db.csv`（种子数据，已拷为 `configs/user_db.csv.example`）、`legacy-new-sim-card-test/`（ATR 样本 + pcap 样本）。
