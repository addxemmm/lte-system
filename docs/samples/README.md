...
- `atr-new-card.txt` / `atr-old-card.txt` — 两批白卡的 ATR（`pcsc_scan` 输出），新批次已在 `configs/sim_profiles.yaml` 登记为 `testsim` 兼容卡 / ATRs (`pcsc_scan` output) of two white-card batches; the new batch is registered in `configs/sim_profiles.yaml` as `testsim` compatible card
