...
| 日志 `UE Authentication Rejected` | `user_db.csv` 的 Key/OPc 与卡内不一致，核对 ue0 行 |
| 反复 attach 失败 | `SQN` 过期：把 ue0 行 `SQN` 改大一点（如 `000000001235`），重启容器再试 |
